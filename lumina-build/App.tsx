...
import { executeBridgeAction, getAIConfig } from './services/aiService';
import { initTheme, loadSavedTheme, applyTheme, getTheme, THEMES } from './themes';
import { FEATURES, IS_PRO, VERSION } from './features';
import { getAIConfig, AIConfig } from './services/aiService';
...
const App = () => {
  ...
  // --- 0.6 SIDEBAR LAYOUT (SmartHome Pro) - PRO ONLY ---
  const currentThemeData = THEMES[currentTheme];
  if (FEATURES.SIDEBAR_LAYOUT && currentThemeData?.layout === 'sidebar') {
      return (
          <SidebarLayout
              rooms={state.rooms}
              devices={state.devices}
              currentRoomId={state.currentRoomId}
              onRoomSelect={(roomId) => setState(s => ({ ...s, currentRoomId: roomId }))}
              onSettingsClick={() => setActiveTab('settings')}
              onDeviceUpdate={handleDeviceUpdate}
              isConnected={connectionStatus.status === 'success'}
              isDemoMode={!configForm.hubIp && !configForm.useCloud}
    ...
    if (activeTab === 'home') {
        ...
        return (
            <div className="flex flex-col items-center w-full animate-in fade-in duration-700 pt-8">
                {/* Favorites Bar (v1.6 Premium) - PRO ONLY */}
                {FEATURES.FAVORITES && favorites.length > 0 && (
            ...
            <div className="animate-in fade-in slide-in-from-bottom-4 duration-500 pb-20">
                ...
                <div className="flex justify-center mt-6">
                    <GlassCard className="w-full max-w-lg p-8 bg-white/5 border-white/10">
                        ...
                        {/* v1.6 Premium: Favorites Editor - PRO ONLY */}
                        {FEATURES.FAVORITES && (
                        ...
                        {/* Tema Visual - PRO ONLY */}
                        {FEATURES.THEME_SYSTEM && (
                        <div className="space-y-4 mt-8 border-t border-white/10 pt-8">
                            <h2 className="text-xl font-light flex items-center gap-2"><Palette size={20} className="text-purple-400" /> Tema Visual</h2>
                            <p className="text-[10px] text-white/60">Escolha o estilo visual do dashboard.</p>
                            <ThemeSelector 
    ...
    <div className="relative min-h-screen w-full bg-black text-white font-sans overflow-hidden">
      ...
      {showAIChat && (
        <AIChat
          ...
          rooms={state.rooms}
          onCommand={async (cmd) => {
            console.log('=== APP ONCOMMAND DEBUG START ===');
            console.log('[App] Command received:', cmd);
            console.log('[App] Command type:', typeof cmd);
            console.log('[App] Command keys:', Object.keys(cmd || {}));

            try {
              console.log('[App] Getting AI config...');
              const aiConfig = getAIConfig();
              console.log('[App] AI config loaded:', { 
                enabled: aiConfig?.enabled,
                provider: aiConfig?.provider,
                bridgeUrl: aiConfig?.bridgeUrl,
                hasBridgeToken: !!aiConfig?.bridgeToken
              });

              console.log('[App] Creating hubitat service wrapper...');
              const hubitatService = {
                executeDeviceCommand: async (deviceId: string, command: string, args: any[] = []) => {
                  console.log('[HubitatService] Execute device command:', { deviceId, command, args });
                  const device = Object.values(state.devices).find(d => 
                    d.id === deviceId || d.hubitatId === deviceId
                  );
                  if (device) {
                    console.log('[HubitatService] Device found:', device.name || device.displayName);
                    
                    // Use correct localStorage key for Hubitat config
                    const hubitatConfigStr = localStorage.getItem('lumina_hubitat_config');
                    console.log('[HubitatService] Hubitat config from localStorage:', hubitatConfigStr);
                    
                    if (!hubitatConfigStr) {
                      throw new Error('Hubitat não configurado - config não encontrada no localStorage');
                    }
                    
                    const hubitatConfig = JSON.parse(hubitatConfigStr);
                    const hubIp = hubitatConfig.hubIp?.replace(/\/$/, ''); // Remove trailing slash
                    const appId = hubitatConfig.appId;
                    const accessToken = hubitatConfig.accessToken;
                    
                    if (!hubIp || !appId || !accessToken) {
                      console.error('[HubitatService] Missing config fields:', { hubIp, appId, accessToken });
                      throw new Error('Configuração do Hubitat incompleta');
                    }
                    
                    const url = `${hubIp}/apps/api/${appId}/devices/${device.hubitatId}/${command}?access_token=${accessToken}`;
                    console.log('[HubitatService] Maker API URL:', url);
                    
                    const response = await fetch(url, { method: 'GET' });
                    if (!response.ok) {
                      throw new Error(`Hubitat API error: ${response.status}`);
                    }
                    
                    const result = await response.text();
                    console.log('[HubitatService] Maker API response:', result);
                  } else {
                    console.error('[HubitatService] Device not found:', deviceId);
                    throw new Error(`Device ${deviceId} not found`);
                  }
                }
              };
              
              console.log('[App] Calling executeBridgeAction...');
              const result = await executeBridgeAction(aiConfig, cmd, hubitatService);
              console.log('[App] executeBridgeAction result:', result);
              ...
              if (result.success) {
                ...
                addNotification({
                  ...
                  title: 'IA - Erro na Execução',
                  message: result.error || 'Comando falhou',
                  type: 'error',
                  timestamp: Date.now()
                });
              }
              ...
              addNotification({
                id: `ai_error_${Date.now()}`,
                title: 'IA - Erro Inesperado',
                message: error.message || 'Erro na execução do comando',
                type: 'error',
                timestamp: Date.now()
              });
            }
